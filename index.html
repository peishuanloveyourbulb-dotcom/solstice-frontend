...
async function autoMemory(userMessage, aiReply, sessionId, modelId) {
  try {
    ...
    var result = '';
    try {
      var memResult = await callModel(autoModel, '你是記憶管理員，負責判斷對話中是否有值得記住的資訊。', [{ role: 'user', content: analyzePrompt }], { temperature: 0.2, maxTokens: 300 });
...
//  🐻 自訂房間自動記憶
// ==========================================
async function autoRoomMemory(userMessage, aiReply, roomId, modelId) {
  try {
    const autoModel = modelId || await getDefaultModel();
    var analyzePrompt = '你是一個對話記憶管理員。請仔細分析以下對話，判斷有沒有值得長期記住的資訊。\n\n' +
      '【一定要記住的】\n' +
      '- 個人喜好：喜歡/討厭/想要的食物、東西、活動、風格\n' +
      '- 生活變化：工作相關、身體狀況、搬家、買東西\n' +
    ...
    result = result.trim();
    if (!result || result === '無' || result.includes('沒有值得') || result.length < 5) {
      console.log('[AutoRoomMemory] Room ' + roomId + ': 沒有需要記住的');
      return;
    }
...
app.patch('/custom-rooms/:id/sessions/:group', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('custom_rooms')
      .update({ session_names: sessionNames }).eq('id', roomId).select().single();
    if (error) throw error;
    res.json({ ok: true, session_names: data.session_names });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/custom-chat', async (req, res) => {
  ...
  try {
    ...
    res.json({ reply: reply, mode: chatMode, usage: usageData });

    // 🐻 背景自動記憶（跟主框邏輯一致）
    if (room_id && chatMode === 'normal' && chatMessages.length > 0) {
      var msgCount = chatMessages.filter(function(m) { return m.role === 'user'; }).length;
      if (msgCount % 2 === 0) {
        autoRoomMemory(userMessage || '[圖片]', reply, room_id, selectedModel).catch(function(e) { console.error('[CustomChat] Auto memory error:', e); });
      }
    }
