...
(function () {
  ...
  function drCamPoke(e) {
    ...
    box.classList.remove('poked'); cam.classList.remove('snap', 'hand', 'reload'); void box.getBoundingClientRect();
    ...
    clearTimeout(drCamTmr); drCamTmr = setTimeout(function () {
      // 收尾不留尾巴（2026/07/17 老婆抓到的）：先讓小拍立得淡出，再趁隱形時無聲收回裝填——
      // 舊寫法直接拿掉 out，紙會被看見「滑回去」那 0.5 秒
      var c = document.getElementById('drCam'); if (!c) return;
      c.classList.add('hand');
      drCamTmr = setTimeout(function () {
        var c2 = document.getElementById('drCam'); if (!c2) return;
        c2.classList.add('reload'); c2.classList.remove('out');
        void c2.getBoundingClientRect();
        c2.classList.remove('reload', 'hand');
      }, 200);
    }, 2700);
  }
