// ============================================
// Sol² 暗房（Darkroom）— darkroom.js v7
// 照片先壓縮再進門（長邊1200 / 品質0.85），
// 取下走兩段式：倉庫實體檔→登記簿，一起徹底清掉 💚
// v7：長按直接拖曳換位置、選單瘦身（更改名稱＋取下）、取景簡化文案
// ============================================
(function () {
  'use strict';
  ...
  var drag = { on: false, id: null, card: null, ghost: null, x: 0, y: 0, moved: false, changed: false, raf: 0 }; // 拖曳換位狀態
      ...
      '<div class="dr-shell">' +
        '<div class="dr-head">' +
          '<div class="dr-hd-left">' +
            ...
            '</div>' +
          '</div>' +
          '<span class="dr-safelight-stk" aria-hidden="true"><svg viewBox="0 0 24 32" style="width:24px;height:30px;display:block"><path d="M12 1 v4.6" stroke="#B9B9C2" stroke-width="1.5" stroke-linecap="round"/><circle cx="12" cy="6.4" r="1.1" fill="none" stroke="#B9B9C2" stroke-width="1.2"/><path d="M5 14.5 a7 7 0 0 1 14 0 z" fill="#2A2A31" stroke="#8C8C96" stroke-width="1.6" stroke-linejoin="round"/><path d="M3.8 14.5 h16.4" stroke="#8C8C96" stroke-width="1.6" stroke-linecap="round"/><g class="dr-bulb"><path d="M6 23.4 l-1.7 1.8 M12 25.2 v2.4 M18 23.4 l1.7 1.8" stroke="#E9A0BC" stroke-width="1.4" stroke-linecap="round"/><circle cx="12" cy="18" r="4.7" fill="#F3B7CA" stroke="#C9698F" stroke-width="1.3"/><circle cx="10.3" cy="17.3" r="0.85" fill="#7E4A5C"/><circle cx="13.7" cy="17.3" r="0.85" fill="#7E4A5C"/><path d="M10.5 19.3 q1.5 1.3 3 0" fill="none" stroke="#7E4A5C" stroke-width="1.1" stroke-linecap="round"/><circle cx="8.5" cy="18.8" r="0.85" fill="#F0A5BE"/><circle cx="15.5" cy="18.8" r="0.85" fill="#F0A5BE"/></g></svg></span>' +
        '</div>' +
        ...
        '<div class="dr-sheet-mask" id="drSheetMask" hidden>' +
          '<div class="dr-sheet dr-stitch" id="drSheet">' +
            ...
            '<div class="dr-sheet-hint">想換位置的話，長按照片直接拖就可以了</div>' +
            '<button class="dr-sheet-btn" data-act="rename">&#9999;&#65039; 更改名稱</button>' +
        ...
        '<div class="dr-modal" id="drRenModal" hidden>' +
          '<div class="dr-ren-card dr-stitch"><span class="dr-tape-ui"></span>' +
            '<div class="dr-ren-t">幫這張照片換一句話</div>' +
            '<input class="dr-cap" id="drRenInput" maxlength="40" placeholder="寫一句話掛在照片下">' +
            '<div class="dr-crop-btns">' +
  ...
  function onWallTouchMove(e) {
    if (drag.on) {
      ...
      if (drag.ghost) drag.ghost.style.transform = 'translate(' + mx + 'px,' + my + 'px) rotate(var(--tilt,0deg)) scale(1.06)';
      hoverReorder();
      return;
    }
  ...
  function hoverReorder() {
    ...
    if (!over || over === drag.card || over.classList.contains('dr-ghost')) return;
    var wall = document.getElementById('drWall');
    if (over.parentNode !== wall) return;
    var kids = Array.prototype.slice.call(wall.children);
    var si = kids.indexOf(drag.card), oi = kids.indexOf(over);
    if (si < 0 || oi < 0 || si === oi) return;
    var prev = [];
    ...
    for (var j = 0; j < kids.length; j++) {
      ...
      void k.offsetWidth;
      k.style.transition = 'transform .22s ease';
  ...
  function dragTick() {
    ...
    var zone = 76, v = 0;
    if (drag.y < r.top + zone) v = -Math.ceil((r.top + zone - drag.y) / 8);
    else if (drag.y > r.bottom - zone) v = Math.ceil((drag.y - (r.bottom - zone)) / 8);
    if (v) { sc.scrollTop += v; hoverReorder(); }
    drag.raf = requestAnimationFrame(dragTick);
  }
