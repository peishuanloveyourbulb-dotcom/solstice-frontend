...
(function () {
  ...
  function drCamPoke(e) {
    ...
    clearTimeout(drCamTmr); drCamTmr = setTimeout(function () {
      // 2026/07/18 收紙改「看得見地縮回去」（老婆指定）：instow 換成不過衝的緩出曲線，
      // 紙慢慢滑回肚子裡。之前的淡出+瞬移是為了躲彈簧回程過衝的紙尖——改曲線就不用躲了。
      var c = document.getElementById('drCam'); if (!c) return;
      c.classList.add('instow');
      c.classList.remove('out');
      drCamTmr = setTimeout(function () {
        var c2 = document.getElementById('drCam'); if (c2) c2.classList.remove('instow');
      }, 560);
    }, 2700);
  }
