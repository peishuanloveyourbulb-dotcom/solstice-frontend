...
(function () {
    ...
    drPage.innerHTML = '' +
      '<div class="dr-shell">' +
        ...
        '<div class="dr-scroll" id="drScroll">' +
          '<div class="dr-plaza" id="drPlaza">' + drBigCam() + '</div>' +
  ...
  function drBigCam() {
    return '<svg class="dr-cam" id="drCam" viewBox="0 0 240 214" aria-label="相機寶寶">' +
      '<defs>' +
      ...
      '<rect x="172" y="52" width="18" height="12" rx="5" fill="#E98FAE" stroke="#B4436F" stroke-width="2.5"/>' +
  ...
  function drCamPoke() {
    ...
    if (!cam) return;
    cam.classList.remove('poked', 'snap'); void cam.getBoundingClientRect();
    cam.classList.add('poked', 'snap', 'squint', 'out'); // 布丁彈＋咔嚓閃光＋瞇眼笑＋吐相片
  ...
  function drLandFirst() {
    ...
    cam.classList.remove('poked', 'snap', 'out', 'hand', 'reload'); void cam.getBoundingClientRect();
    cam.classList.add('poked', 'snap', 'squint', 'out'); // ① 咔嚓＋吐相片
